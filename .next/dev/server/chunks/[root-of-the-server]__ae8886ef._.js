...
"[project]/pages/api/seed.js [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
    ...
    try {
        ...
        try {
            ...
            return res.status(500).json({
                ...
                details: String(connErr && connErr.message ? connErr.message : connErr)
            });
        }
        ...
        const desiredCampaigns = [
            ...
            {
                name: 'Technology Access Campaign',
                goal: 60000
            },
            {
                name: 'Holiday Giving Drive 2025',
                goal: 90000
            }
        ];
        ...
        const approvedCampaignNames = new Set([
            ...
            'Holiday Giving Drive 2025'
        ]);
        for (const cd of desiredCampaigns){
            try {
                let c = await prisma.campaign.findFirst({
                    where: {
                        name: cd.name
                    }
                });
                if (!c) {
                    c = await prisma.campaign.create({
                        data: {
                            name: cd.name,
                            goal: cd.goal,
                            approved: approvedCampaignNames.has(cd.name)
                        }
                    });
                } else {
        ...
        const donor1 = await prisma.donor.upsert({
            where: {
                email: 'nate.marshall@example.com'
            },
            update: {},
            create: {
                firstName: 'Nate',
                ...
                email: 'nate.marshall@example.com',
                totalGiving: 12500,
                lastGiftAt: new Date('2025-03-03')
            }
        });
        const donor2 = await prisma.donor.upsert({
            ...
            create: {
                ...
                email: 'evelyn.hart@example.com',
                totalGiving: 4200,
                lastGiftAt: new Date('2025-01-12')
            }
        });
        ...
        const extraDonorsData = [
            {
                ...
                totalGiving: 1200,
                lastGiftAt: new Date('2025-11-01')
            },
            {
                firstName: 'Liam',
                lastName: 'Smith',
                ...
                totalGiving: 500,
                lastGiftAt: new Date('2025-09-18')
            },
            {
                firstName: 'Olivia',
                lastName: 'Garcia',
                ...
                totalGiving: 300,
                lastGiftAt: new Date('2025-08-10')
            },
            {
                firstName: 'Noah',
                lastName: 'Johnson',
            ...
            },
            {
                ...
                totalGiving: 750,
                lastGiftAt: new Date('2025-07-04')
            },
            {
                firstName: 'Mason',
                lastName: 'Brown',
                ...
                totalGiving: 2500,
                lastGiftAt: new Date('2025-06-21')
            },
            {
                firstName: 'Isabella',
                lastName: 'Davis',
            ...
            },
            {
                ...
                totalGiving: 1500,
                lastGiftAt: new Date('2025-05-30')
            }
        ];
        const extraDonors = [];
        for (const d of extraDonorsData){
            try {
                const up = await prisma.donor.upsert({
                    where: {
                        email: d.email
                    },
                    update: {},
                    create: {
                        ...
                        lastName: d.lastName,
                        email: d.email,
                        totalGiving: d.totalGiving || 0,
                        lastGiftAt: d.lastGiftAt || null
                    }
                });
                extraDonors.push(up);
            } catch (e) {
        ...
        const seededPasswordHash = await __TURBOPACK__imported__module__$5b$externals$5d2f$bcryptjs__$5b$external$5d$__$28$bcryptjs$2c$__cjs$29$__["default"].hash('password123', 10);
        const adminUser = await prisma.user.upsert({
            where: {
                email: 'rob.admin@example.com'
            },
            update: {},
            create: {
                name: 'Rob Admin',
        ...
        try {
            for (const name of approvedCampaignNames){
                ...
                if (!c) continue;
                const donationsForCampaign = await prisma.donation.count({
                    where: {
                        campaignId: c.id
                    }
                });
        ...
        try {
            if (campaignHoliday && campaignHoliday.id) {
                const boostExists = await prisma.donation.findFirst({
                    where: {
                        campaignId: campaignHoliday.id,
                        method: 'SEED-BOOST'
                    }
                ...
                if (!boostExists) {
                    ...
                    if (booster) {
                        try {
                            ...
                            console.warn('Seed: failed to create boost donation', e);
                        }
                    }
                }
            }
        } catch (e) {
            console.warn('Seed: boost donation check failed', e);
        }
        // Recalculate donor totals and lastGiftAt based on seeded donations so DB reflects actual totals
        try {
            const donor1Agg = await prisma.donation.aggregate({
                where: {
                    donorId: donor1.id
                },
                _sum: {
            ...
            const donor2Agg = await prisma.donation.aggregate({
                where: {
                    donorId: donor2.id
                },
                _sum: {
        ...
        const campaignsWithDonations = await prisma.campaign.findMany({
            include: {
                donations: true
            }
        });
        const campaignStats = campaignsWithDonations.map((c)=>{
            ...
            const gifted = c.donations.filter((d)=>{
                try {
        ...
        const donorStats = donorsWithDonations.map((d)=>({
                ...
                email: d.email,
                totalGiving: d.donations.reduce((s, x)=>s + (Number(x.amount || 0) || 0), 0),
                giftedTotal: d.donations.filter((dd)=>{
                    try {
                        const m = String(dd.method || '').toLowerCase();
                        const n = String(dd.notes || '').toLowerCase();
                        return /gift/.test(m) || /gift/.test(n);
                    } catch (e) {
                        return false;
                    }
        ...
        const metricsPayload = [
            ...
            },
            {
                ...
                value: String(campaignStats && campaignStats.length || 0),
                sub: 'Running programs'
            },
            {
                title: 'Total Revenue',
                value: '$' + (totalRevenue && totalRevenue.toLocaleString ? totalRevenue.toLocaleString() : String(totalRevenue || 0)),
                sub: 'From all programs'
            },
            {
                title: 'Total Donors',
                value: String(totalDonors || 0),
                sub: 'Individuals who gave'
            },
            {
        ...
        const followUpsSeed = (campaignsWithDonations || []).slice(0, 3).map((c, i)=>({
                id: `f${i + 1}`,
                campaign: {
                    id: c.id,
                    name: c.name
        ...
        const payload = {
                    ...
                    interactions: [
                        {
                            ...
                            type: 'Email',
                            date: '2025-11-01',
                            note: 'Left proposal for holiday event'
                        },
                        {
                            id: 'i1b',
                            type: 'Call',
                            date: '2025-12-05',
                            note: 'Discussed in-store activation logistics for holiday weekend'
                        }
                    ],
                    programs: [],
                    reminders: [
                        {
                            id: 'r-gs-1',
                            title: 'Holiday activation follow-up',
                            due: '2025-12-10',
                            note: 'Confirm in-store setup and co-marketing materials'
                        }
                    ]
                },
                {
                    ...
                    interactions: [
                        {
                            ...
                            type: 'Call',
                            date: '2025-11-28',
                            note: 'Sent proposal draft'
                        },
                        {
                            id: 'i2b',
                            type: 'Email',
                            date: '2025-12-02',
                            note: 'Requested technical integration specs'
                        }
                    ],
                    programs: [],
                    reminders: [
                        {
                            id: 'r-dis-1',
                            title: 'Integration specs follow-up',
                            due: '2025-12-08',
                            note: 'Confirm API access and streaming integration windows'
                        }
                    ]
                },
                {
                    ...
                    interactions: [
                        ...
                        },
                        {
                            ...
                            type: 'Call',
                            date: '2025-08-20',
                            note: 'Checked installation schedule and on-site support'
                        }
                    ],
                    programs: [],
                    reminders: [
                        {
                            id: 'r-al-1',
                            title: 'Post-installation check-in',
                            due: '2025-12-14',
                            note: 'Verify final setup and collect feedback from venue team'
                        }
                    ]
                },
                {
                    ...
                    contacts: [
                        {
                            name: 'Sarah Martinez',
                            title: 'Esports Partnership Manager',
                            email: 'sarah.martinez@redbull.com',
                            phone: '555-0123'
                        }
                    ],
                    interactions: [
                        ...
                        },
                        {
                            ...
                            type: 'Email',
                            date: '2025-01-10',
                            note: 'Sent post-event metrics and initial renewal proposal.'
                        },
                        {
                            id: 'i6',
                            type: 'Call',
                            date: '2025-06-15',
                            note: 'Follow-up call to review renewal options and expanded activation.'
                        }
                    ],
                    programs: [
                        {
                            id: 'p1',
                            title: 'Red Bull Tournament Series 2024',
                            status: 'Active',
                            value: '$500,000',
                            start: '2024-01-14',
                            end: '2025-01-13'
                    ...
                    reminders: [
                        {
                            ...
                            due: '2025-12-14',
                            note: 'Reach out to discuss 2025 renewal terms and potential expansion'
                        }
                    ],
            ...
            donors: [
                {
                    name: `${donor1.firstName} ${donor1.lastName}`,
                    totalGiving: '$12,500',
                    lastGift: 'Mar 3, 2025'
                },
                {
                    name: `${donor2.firstName} ${donor2.lastName}`,
                    totalGiving: '$4,200',
...
//# sourceMappingURL=%5Broot-of-the-server%5D__ae8886ef._.js.map