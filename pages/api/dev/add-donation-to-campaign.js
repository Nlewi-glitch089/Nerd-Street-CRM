...
export default async function handler(req, res) {
  ...
  try {
    ...
    const campaign = await prisma.campaign.findFirst({ where: { name: campaignName } })
    ...
    if (!campaign.approved && raised > 0) {
      try { await prisma.campaign.update({ where: { id: campaign.id }, data: { approved: true } }) } catch (e) { console.warn('Failed to update campaign approved flag', e) }
    }
